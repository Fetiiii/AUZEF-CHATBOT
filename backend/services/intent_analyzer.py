...
def build_intent_analyzer_prompt(
    ...
    system = (
        ...
        "1) Current user message tek başına self-contained mı, yani neyi "
        "sorduğu previous_user_turns'e HİÇ bakmadan anlaşılıyor mu? "
        "2) Self-contained ise context_used = false ve resolved_text = "
        "normalized_text BİREBİR AYNI olmalıdır: normalized_text'i aynen "
        "kopyala; yeniden ifade etme, özetleme, soruyu cümleye çevirme ya da "
        "'bilgi almak istiyorsunuz' gibi açıklama üretme. "
        "3) Self-contained değilse previous_user_turns'e bak: referent orada "
        "tek anlamlı biçimde belirleniyorsa context_used = true olur. "
        "4) Belirlenmiyorsa context_used = false kalır ve resolved_text = "
        "normalized_text olur; context uydurma.\n"
        # ── Failure class 2: reference-dependent short questions ────────────
        "SELF-CONTAINED DEĞİLDİR TESTİ: Bir soru, içindeki referent — isim, "
        "özne, nesne, işlem, başvuru, belge kümesi, tarih, ücre, süre ya da yer "
        "— YALNIZ previous_user_turns'ten belirlenebiliyorsa self-contained "
        "değildir. 'Şartı', 'belgesi', 'ücreti', 'süresi', 'tarihi', 'yeri' gibi "
        "bir niteleyici HANGİ konuya ait olduğunu söylemiyorsa referent eksiktir "
        "ve current message tek başına cevaplanamaz. Aynı şey zamir ('onu', "
        "'bunun'), eksilti ve 'peki ...?' ile başlayan kısa follow-up'lar için de "
        "geçerlidir. "
        "Sentetik örnek: previous 'Staj başvurusu yapmak istiyorum', current "
        "'Nereden yapılıyor?' → referent ('staj başvurusu') yalnız önceki "
        "turn'den bilinir → context_used true, resolved_text 'Staj başvurusu "
        "nereden yapılıyor?'.\n"
        "GRAMER YETERLİ DEĞİLDİR: Gramer olarak tamamlanmış bir soru semantik "
        "olarak self-contained olmayabilir. Belgeler, evraklar, gereklilikler, "
        "şartlar, tarihler, ücretler, adımlar, koşullar, sonuçlar veya benzeri "
        "genel bir isim ya da isim öbeğinin HANGİ konu, işlem, başvuru veya "
        "sürece ait olduğu current message içinde açıkça belirtilmiyorsa soru "
        "self-contained değildir. 'Hangi X?', 'X'ler neler?' veya 'X gerekiyor "
        "mu?' gibi yapılarda X bir kategori/set ise, referenti current message'den "
        "belirleme; previous_user_turns'e bak. Previous USER turn referenti tek "
        "anlamlı biçimde sağlıyorsa context_used = true yap ve resolved_text'e "
        "açık referenti ekle. Current message zaten konuyu/süreci belirtiyorsa "
        "context_used = false kalır. Previous USER turn referenti tek anlamlı "
        "sağlamıyorsa context_used = false kalır; başvuru türü, program, işlem "
        "veya kategori uydurma.\n"
        # ── Failure class 1: the flag and the rewrite are one decision ──────
        "context_used = true DEMEK, resolved_text'i current isteği "
        "previous_user_turns'e ihtiyaç duymadan anlaşılır kılacak şekilde YENİDEN "
        "YAZMAK demektir; bu ikisi tek bir karardır, ayrı ayrı seçilemez. "
        "resolved_text'i hâlâ çözümlenmemiş ya da referanslı halde bırakıp "
        "context_used = true DEME. Eksik referenti önceki USER turn'den ekle; "
        "yalnız gerekli bilgiyi taşı, gereksiz paraphrase yapma ve bu iki "
        "kaynakta geçmeyen kelime ekleme. "
        "Ölçüt mekanik değil semantiktir: resolved_text'i tek başına okuyan biri "
        "neyin sorulduğunu anlayabilmelidir.\n"
        # ── Anti-over-trigger (protects the 4/4 control result) ─────────────
        "AŞIRI TETİKLEME YOK: Kısa olmak tek başına context-dependent olmak "
        "demek değildir. Current message kendi başına açıksa — referenti kendi "
        "içinde taşıyorsa — geçmiş görünür olsa bile context_used = false kalır "
        "ve resolved_text = normalized_text olur. Açık ve tek başına anlaşılan "
        "bir current turn'e eski konuyu taşıma. "
