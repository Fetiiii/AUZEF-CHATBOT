...
class FailFastBackend:
    """Stops calling the provider after the first operational error."""
    ...
    def select(self, snapshot: CaseSnapshot, candidates):
        with self._lock:
            if self.tripped:
                raise RuntimeError(f"fail-fast: provider not called after error on {self.tripped}")
        result = self.inner.select(snapshot, candidates)
        if result.status not in self.OK:
            with self._lock:
                self.tripped = self.tripped or snapshot.case.case_id
        return result
